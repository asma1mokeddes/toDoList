...
describe("Test for sending emails", () => {
    it("Check email sending when adding the 8th item to the ToDo list", () => {
        const mailer = new EmailSenderService();
        let toDo = new ToDo(mailer);
        ...
        toDo.emailSenderService.send = jest.fn((content) => {
            console.log(content);
        });

        let date = new Date();
        date.setMinutes(date.getMinutes() + 35);
        let item8 = new Item("Item 8", "Contenu 8", date);

        toDo.add(item8);

        expect(toDo.emailSenderService.send).toHaveBeenCalledWith(
